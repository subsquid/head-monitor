...
export interface PortalApi {
  kind: 'portal_api';
  dataset_kind: 'evm' | 'solana' | 'bitcoin' | 'tron' | 'hyperliquid-fills' | 'hyperliquid-replica-cmds';
  url: string;
}
...
function validateMeasurement(measurement: any, datasetName: string, measurementName: string): asserts measurement is Measurement {
  ...
  if (!measurement.target.dataset_kind || !['evm', 'solana', 'bitcoin', 'tron', 'hyperliquid-fills', 'hyperliquid-replica-cmds'].includes(measurement.target.dataset_kind)) {
    throw new Error(`Invalid measurement ${measurementName} in dataset ${datasetName}: target dataset_kind must be 'evm' or 'solana' or 'bitcoin' or 'tron' or 'hyperliquid-fills' or 'hyperliquid-replica-cmds'`);
  }
}
